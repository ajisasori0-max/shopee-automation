...
ADS_PARTNER_ID = _ads["partner_id"]
ADS_PARTNER_KEY = _ads["partner_key"]

# SELLER APP Config (for product boost)
SELLER_PARTNER_ID = _seller["partner_id"]
SELLER_PARTNER_KEY = _seller["partner_key"]
...
def get_token(token_file, partner_id, partner_key):
    """Get valid token via central token authority."""
    app_name = 'ads' if 'ads' in token_file else 'production'
    return get_access_token(app_name), None

def refresh_token(token_file, partner_id, partner_key):
    """Refresh access token via central token authority."""
    app_name = 'ads' if 'ads' in token_file else 'production'
    return get_access_token(app_name, force_refresh=True)
...
def make_request(path, params=None, method='GET', body=None, token_file=None, partner_id=None, partner_key=None):
    ...
    access_token = get_token(token_file, partner_id, partner_key)[0]
    if not access_token:
